...
const ORIGENS_INCENDIO = [
  ...
  {
    value: "REDE_ELETRICA",
    label: "Problemas na rede elétrica (curto circuito, cabo rompido, transformador, etc.)",
  },
...
const RESULTADOS = [
  {
    value: "EM_ANDAMENTO",
    label: "Em andamento",
    desc: "Combate ativo no momento do preenchimento, sem extinção total.",
  },
  {
    value: "NECESSIDADE_FISCALIZACAO",
    label: "Necessidade de equipe de fiscalização",
    desc: "Ex: queima controlada ou intencional, fogo já extinto, situação monitorada.",
  },
  {
    value: "SEM_INTERVENCAO",
    label: "Sem necessidade de intervenção",
    desc: "Incêndio extinto sozinho ou por terceiros antes da chegada da equipe.",
  },
  {
    value: "EXTINTO_RESOLVIDA",
    label: "Incêndio extinto / Resolvida",
    desc: "Equipe concluiu o combate e confirmou extinção completa do fogo.",
  },
  {
    value: "DESPACHO_INCORRETO",
    label: "Despacho incorreto",
    desc: "Não foi encontrado nenhum incêndio ou queimada nas imediações.",
  },
...
export function RelatorioTerrestreForm({
  ...
  // ── Área de Atuação ──
  const [areaAtuacaoLat, setAreaAtuacaoLat] = useState<number | undefined>(
    initialData?.areaAtuacaoLat,
  );
  const [areaAtuacaoLng, setAreaAtuacaoLng] = useState<number | undefined>(
    initialData?.areaAtuacaoLng,
  );

  // ── Ações de Combate ──
  const [acoesRealizadas, setAcoesRealizadas] = useState<string[]>(
    initialData?.acoesRealizadas ?? [],
  );

  // ── Órgãos de Apoio ──
  const [orgaosApoio, setOrgaosApoio] = useState<string[]>(initialData?.orgaosApoio ?? []);
  const [outrosOrgaosDescricao, setOutrosOrgaosDescricao] = useState(
    initialData?.outrosOrgaosDescricao ?? "",
  );

  const [houveUsoAgua, setHouveUsoAgua] = useState<boolean | undefined>(
    initialData?.houveUsoAgua !== undefined ? initialData?.houveUsoAgua : undefined,
  );
  const [volumeAguaLitros, setVolumeAguaLitros] = useState(
    initialData?.volumeAguaLitros?.toString() ?? "",
  );
  const [origensAgua, setOrigensAgua] = useState<string[]>(initialData?.origensAgua ?? []);
  const [outraOrigemAguaDescricao, setOutraOrigemAguaDescricao] = useState(
    initialData?.outraOrigemAguaDescricao ?? "",
  );

  // ── Apoio Rural ──
  const [houveApoioPropriedades, setHouveApoioPropriedades] = useState<boolean>(
    initialData?.houveApoioPropriedades ?? false,
  );
  ...
  // ── Recusa Rural ──
  const [houveRecusaPropriedades, setHouveRecusaPropriedades] = useState<boolean>(
    initialData?.houveRecusaPropriedades ?? false,
  );
  ...
  // ── Origem do Incêndio ──
  const [possivelOrigemIncendio, setPossivelOrigemIncendio] = useState(
    initialData?.possivelOrigemIncendio ?? "",
  );
  const [outraOrigemDescricao, setOutraOrigemDescricao] = useState(
    initialData?.outraOrigemDescricao ?? "",
  );

  // ── Efetividade ──
  const [efetividadeCombate, setEfetividadeCombate] = useState(
    initialData?.efetividadeCombate ?? "",
  );

  // ── Reforço ──
  const [necessidadeReforco, setNecessidadeReforco] = useState<boolean | undefined>(
    initialData?.necessidadeReforco !== undefined ? initialData?.necessidadeReforco : undefined,
  );
  const [tiposReforcoNecessarios, setTiposReforcoNecessarios] = useState<string[]>(
    initialData?.tiposReforcoNecessarios ?? [],
  );

  // ── Histórico e Resultado ──
  const [historicoDescritivo, setHistoricoDescritivo] = useState(
    initialData?.historicoDescritivo ?? "",
  );
  const [resultadoOcorrencia, setResultadoOcorrencia] = useState(
    initialData?.resultadoOcorrencia ?? "",
  );
  const [outroResultadoDescricao, setOutroResultadoDescricao] = useState(
    initialData?.outroResultadoDescricao ?? "",
  );
  ...
  const validate = (): ValidationErrors => {
    ...
    if (orgaosApoio.includes("OUTROS") && !outrosOrgaosDescricao.trim())
      errs.outrosOrgaos = "Descreva os outros órgãos de apoio.";
    if (!possivelOrigemIncendio) errs.origem = "Selecione a possível origem do incêndio.";
    if (possivelOrigemIncendio === "OUTRO" && !outraOrigemDescricao.trim())
      errs.outraOrigem = "Descreva a outra origem.";
    ...
    if (historicoDescritivo.trim().length < 20)
      errs.historico = "O histórico deve ter pelo menos 20 caracteres.";
    ...
    if (houveUsoAgua === true && (!volumeAguaLitros || Number(volumeAguaLitros) <= 0))
      errs.volumeAgua = "Informe o volume de água utilizado.";
    if (!areaAtuacaoLat || !areaAtuacaoLng)
      errs.areaAtuacao = "Selecione a área de atuação no mapa.";
    if (necessidadeReforco === undefined)
      errs.necessidadeReforco = "Selecione se há necessidade de reforço.";
    if (necessidadeReforco === true && tiposReforcoNecessarios.length === 0)
      errs.reforcos = "Selecione pelo menos um tipo de reforço.";
  ...
  const handleLocalSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload: RelatorioTerrestrePayload = {
      ...
      volumeAguaLitros:
        houveUsoAgua === true && volumeAguaLitros ? Number(volumeAguaLitros) : undefined,
      origensAgua: houveUsoAgua === true ? origensAgua : undefined,
      outraOrigemAguaDescricao:
        houveUsoAgua === true && origensAgua.includes("OUTRO")
          ? outraOrigemAguaDescricao
          : undefined,
      ...
      outroResultadoDescricao:
        resultadoOcorrencia === "OUTRO" ? outroResultadoDescricao : undefined,
    };
  ...
  return (
    <form id="form-terrestre" onSubmit={handleLocalSubmit} className="space-y-6 p-1" noValidate>
      {/* ── Ações de Combate ── */}
      <div className={sectionClass(!!errors.acoes)} data-error={submitted && !!errors.acoes}>
          ...
          {ACOES_COMBATE.map((a) => (
            <div key={a.value} className="flex items-start space-x-2">
              <Checkbox
                ...
                onCheckedChange={() =>
                  !readOnly && toggleSet(a.value, acoesRealizadas, setAcoesRealizadas)
                }
              />
              <label
                htmlFor={`acao-${a.value}`}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {ORGAOS_APOIO.map((o) => (
            <div key={o.value} className="flex items-start space-x-2">
              ...
              />
              <label htmlFor={`orgao-${o.value}`} className="text-sm leading-none cursor-pointer">
                {o.label}
              </label>
            </div>
          ))}
      ...
      {/* ── Área de Atuação ── */}
      <div
        className={sectionClass(!!errors.areaAtuacao)}
        data-error={submitted && !!errors.areaAtuacao}
      >
        ...
        <div className="relative rounded-lg overflow-hidden border border-border h-[350px]">
          <SituationMapClient
            ...
            activePin={
              areaAtuacaoLat && areaAtuacaoLng ? { lat: areaAtuacaoLat, lng: areaAtuacaoLng } : null
            }
            onClickMap={(lat, lng) => {
              if (!readOnly) {
      ...
      {/* ── Uso de Água ── */}
      <div
        className={sectionClass(!!errors.usoAgua || !!errors.volumeAgua)}
        data-error={submitted && (!!errors.usoAgua || !!errors.volumeAgua)}
      >
                ...
                {ORIGENS_AGUA.map((o) => (
                  <div key={o.value} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={() =>
                        !readOnly && toggleSet(o.value, origensAgua, setOrigensAgua)
                      }
                    />
                    <label
                      htmlFor={`agua-${o.value}`}
              ...
              <div key={p.id} className="glass p-4 rounded-lg space-y-3 relative">
                {!readOnly && propriedadesApoio.length > 1 && (
                  <Button
                    ...
                    onClick={() =>
                      setPropriedadesApoio((prev) => prev.filter((x) => x.id !== p.id))
                    }
                  >
                ...
                <Button
                  ...
                  onClick={() =>
                    setPropriedadesApoio((prev) =>
                      prev.map((x) => (x.id === p.id ? { ...x, mapOpen: !x.mapOpen } : x)),
                    )
                  }
                >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setPropriedadesApoio((prev) =>
                          prev.map((x) =>
                            x.id === p.id ? { ...x, responsavel: e.target.value } : x,
                          ),
                        )
                      }
                    />
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setPropriedadesApoio((prev) =>
                          prev.map((x) => (x.id === p.id ? { ...x, telefone: e.target.value } : x)),
                        )
                      }
                    />
                  </div>
                      ...
                      <div key={tipo} className="flex items-center gap-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setPropriedadesApoio((prev) =>
                              prev.map((x) =>
                                x.id === p.id ? { ...x, tipoApoio: checked ? tipo : "" } : x,
                              ),
                            )
                          }
                        />
                        ...
                        {p.tipoApoio === tipo && tipo !== "OUTRO" && (
                          <Input
                            ...
                            onChange={(e) =>
                              setPropriedadesApoio((prev) =>
                                prev.map((x) =>
                                  x.id === p.id ? { ...x, quantidadeApoio: e.target.value } : x,
                                ),
                              )
                            }
                          />
                        ...
                        {p.tipoApoio === tipo && tipo === "OUTRO" && (
                          <Input
                            ...
                            onChange={(e) =>
                              setPropriedadesApoio((prev) =>
                                prev.map((x) =>
                                  x.id === p.id ? { ...x, descricaoApoioOutro: e.target.value } : x,
                                ),
                              )
                            }
                          />
      ...
      <div className="space-y-4">
        ...
        <p className="text-xs text-muted-foreground">
          Algum proprietário rural se recusou a colaborar com a operação de combate?
        </p>
        <RadioGroup
              ...
              <div key={p.id} className="glass p-4 rounded-lg space-y-3 relative">
                {!readOnly && propriedadesRecusa.length > 1 && (
                  <Button
                    ...
                    className="absolute top-2 right-2 h-6 w-6 text-destructive"
                    onClick={() =>
                      setPropriedadesRecusa((prev) => prev.filter((x) => x.id !== p.id))
                    }
                  >
                    <Trash2 className="h-3 w-3" />
                  </Button>
                      ...
                      <div key={m.value} className="flex items-start space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setPropriedadesRecusa((prev) =>
                              prev.map((x) =>
                                x.id === p.id ? { ...x, motivoRecusa: checked ? m.value : "" } : x,
                              ),
                            )
                          }
                        />
                        <label
                          htmlFor={`recusa-motivo-${p.id}-${m.value}`}
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setPropriedadesRecusa((prev) =>
                                prev.map((x) =>
                                  x.id === p.id
                                    ? { ...x, descricaoRecusaOutro: e.target.value }
                                    : x,
                                ),
                              )
                            }
      ...
      >
        ...
        <RadioGroup
          value={
            necessidadeReforco !== undefined ? (necessidadeReforco ? "sim" : "nao") : undefined
          }
        ...
        {necessidadeReforco && (
          <div
            className={cn(
              "grid grid-cols-1 sm:grid-cols-2 gap-3 pl-6 border-l-2 border-warning/30 pt-2",
              submitted &&
                !!errors.reforcos &&
            ...
            {[
              ...
              <div key={r.value} className="flex items-start space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={() =>
                    !readOnly &&
                    toggleSet(r.value, tiposReforcoNecessarios, setTiposReforcoNecessarios)
                  }
                />
                <label htmlFor={`ref-${r.value}`} className="text-sm cursor-pointer">
                  {r.label}
                </label>
              </div>
            ))}
      ...
      >
        ...
        <p className="text-xs text-muted-foreground text-right">
          {historicoDescritivo.length} caracteres (mín. 20)
        </p>
          ...
          {RESULTADOS.map((r) => (
            <div key={r.value} className="flex items-start space-x-2">
              ...
              <div className="grid gap-1.5 leading-none">
                <Label htmlFor={`res-${r.value}`} className="cursor-pointer">
                  {r.label}
                </Label>
              ...
              {r.value === "OUTRO" && resultadoOcorrencia === "OUTRO" && (
                <Input
                  className={cn(
                    "h-8 flex-1 ml-2",
                    submitted && errors.outroResultado && "border-destructive",
                  )}
