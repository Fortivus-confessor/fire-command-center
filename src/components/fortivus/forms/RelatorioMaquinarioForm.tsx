...
export function RelatorioMaquinarioForm({
  ...
  const [resultado, setResultado] = useState<string>(
    initialData?.resultadoOcorrencia || "andamento",
  );
  const [empregoAceiro, setEmpregoAceiro] = useState(
    initialData?.tiposEmprego?.includes("Confecção de aceiros") || false,
  );
  const [empregoOutro, setEmpregoOutro] = useState(
    initialData?.tiposEmprego?.includes("Outro") || false,
  );
  ...
  const handleLocalSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.querySelector('#emp-aceiros[data-state="checked"]'))
      tiposEmprego.push("Confecção de aceiros");
    if (form.querySelector('#emp-fogo[data-state="checked"]'))
      tiposEmprego.push("Apoio a fogo contrafogo");
    ...
    const payload = {
      horimetroInicial: data.get("horimetroInicial")
        ? parseFloat(data.get("horimetroInicial") as string)
        : null,
      horimetroFinal: data.get("horimetroFinal")
        ? parseFloat(data.get("horimetroFinal") as string)
        : null,
      tempoLiquido: data.get("tempoLiquido") as string,
      horaInicioOperacao: data.get("horaInicioOperacao")
        ? data.get("horaInicioOperacao") + ":00"
        : null,
      horaFimOperacao: data.get("horaFimOperacao") ? data.get("horaFimOperacao") + ":00" : null,
      tiposEmprego,
      comprimentoAceiros:
        empregoAceiro && data.get("comprimentoAceiros")
          ? parseFloat(data.get("comprimentoAceiros") as string)
          : null,
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Ex: 5400.5"
              defaultValue={initialData?.horimetroInicial}
              step="0.1"
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              type="number"
              name="horimetroFinal"
              placeholder="Ex: 5408.2"
              defaultValue={initialData?.horimetroFinal}
              step="0.1"
            />
          </div>
          <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
          <div className="space-y-2">
            ...
            <Input
              type="time"
              name="horaInicioOperacao"
              defaultValue={initialData?.horaInicioOperacao?.substring(0, 5)}
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              type="time"
              name="horaFimOperacao"
              defaultValue={initialData?.horaFimOperacao?.substring(0, 5)}
            />
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex flex-wrap gap-4">
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                id="emp-aceiros"
                checked={empregoAceiro}
                onCheckedChange={(c) => setEmpregoAceiro(!!c)}
              />
              <label htmlFor="emp-aceiros" className="text-sm font-medium leading-none">
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                id="emp-fogo"
                defaultChecked={initialData?.tiposEmprego?.includes("Apoio a fogo contrafogo")}
              />
              <label htmlFor="emp-fogo" className="text-sm font-medium leading-none">
                Apoio a fogo contrafogo
              </label>
            </div>
            <div className="flex items-center space-x-2">
              <Checkbox
                id="emp-outro"
                checked={empregoOutro}
                onCheckedChange={(c) => setEmpregoOutro(!!c)}
              />
              <label htmlFor="emp-outro" className="text-sm font-medium leading-none">
                Outro
              </label>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mt-2">
            {empregoAceiro && (
              <div className="space-y-2 flex-1 max-w-xs">
                <Label>
                  Comprimento dos aceiros construídos (m){" "}
                  <span className="text-destructive">*</span>
                </Label>
                <Input
                  type="number"
                  name="comprimentoAceiros"
                  placeholder="Ex: 500"
                  required={empregoAceiro}
                  defaultValue={initialData?.comprimentoAceiros}
                  step="0.1"
                />
              </div>
            )}
            {empregoOutro && (
              <div className="space-y-2 flex-1 max-w-xs">
                ...
                <Input
                  name="descricaoOutroEmprego"
                  placeholder="Descreva..."
                  required
                  defaultValue={initialData?.descricaoOutroEmprego}
                />
              </div>
            )}
          </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Textarea
            ...
            placeholder="Descreva de forma livre os detalhes da operação com maquinário..."
