...
export function AttachmentUploader({
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        const presignedRes = await fetchWithAuth(
          `/attachments/upload-url?fileName=${encodeURIComponent(file.name)}&contentType=${encodeURIComponent(file.type)}`,
        );
  ...
  return (
    <div className="space-y-4 border rounded-lg p-4 bg-background/50 border-border">
      <div
        className="flex flex-col items-center justify-center border-2 border-dashed border-border/60 rounded-lg p-6 hover:bg-secondary/10 transition cursor-pointer"
        ...
        <div className="space-y-2">
          ...
          <ul className="space-y-2">
            {files.map((file, i) => (
              <li
                key={i}
                className="flex items-center justify-between bg-secondary/20 p-2 rounded text-sm"
              >
                <div className="flex items-center gap-2 overflow-hidden">
                  {file.type.startsWith("image/") ? (
                    <ImageIcon className="h-4 w-4 text-fire" />
                  ) : (
                    <File className="h-4 w-4 text-command" />
                  )}
          ...
          <div className="pt-2 flex flex-col gap-2">
            {uploading && (
              <div className="w-full bg-secondary rounded-full h-2.5">
                <div
                  className="bg-fire h-2.5 rounded-full transition-all"
