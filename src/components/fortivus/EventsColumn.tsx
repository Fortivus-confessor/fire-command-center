  ...
  return (
    <section className="glass rounded-xl flex flex-col h-full min-h-0">
      <header className="px-3 sm:px-4 pt-3 pb-2 border-b border-border">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-[10px] mono text-muted-foreground border border-border rounded px-1.5 py-0.5">INBOX · {focos.length}</span>
          </div>
      ...
      <div className="overflow-y-auto flex-1 min-h-0 divide-y divide-border">
        {isLoading ? (
           <div className="text-center text-muted-foreground text-sm py-4">Buscando focos no radar...</div>
        ) : focos.length === 0 ? (
          <div className="text-center text-muted-foreground text-sm py-4">Nenhum evento detectado.</div>
        ) : (
          focos.map((f: any) => {
            ...
            return (
              ...
              >
                {active && <span className="absolute left-0 top-2 bottom-2 w-0.5 bg-fire rounded-r" />}
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="mono text-[11px] text-muted-foreground">{f.externalId?.substring(0,10) || f.id.substring(0,8)}</span>
                      <span className={`text-[10px] uppercase tracking-wider border rounded px-1.5 py-0.5 ${f.frp > 100 ? riskBadge.CRITICO : riskBadge.MEDIO}`}>{f.frp > 100 ? 'EXTREMO' : 'ALTO'}</span>
                    </div>
                    <p className="text-sm font-medium mt-0.5 truncate">NASA FIRMS <span className="text-muted-foreground font-normal">/ Satélite</span></p>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-[10px] text-muted-foreground mono">{new Date(f.acquisitionDate).toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'})}</p>
                    <p className="text-[11px] mono mt-0.5">{f.confidence != null ? f.confidence : '--'}</p>
                  </div>
                </div>
                <div className="mt-2 flex items-center justify-between text-[10px] mono text-muted-foreground">
                  ...
                  <span>{f.latitude?.toFixed(3)}, {f.longitude?.toFixed(3)}</span>
                </div>
              </button>
            );
          })
