    ...
    <section className="glass rounded-xl flex flex-col h-full min-h-0">
      <header className="px-3 sm:px-4 pt-3 pb-2 border-b border-border">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-[10px] mono text-muted-foreground border border-border rounded px-1.5 py-0.5">PLANEJAMENTO</span>
          </div>
      ...
      {fire && (
        <div className="mx-3 sm:mx-4 mt-3 rounded-lg border border-dashed border-command/40 bg-command/5 p-3">
          <div className="flex items-center justify-between gap-2">
            <p className="text-[10px] uppercase tracking-widest text-command mono">Criar OS a partir do evento</p>
            <span className="mono text-[10px] text-muted-foreground">{fire.codigo}</span>
          </div>
          <p className="text-sm font-medium mt-1">{fire.municipio}/{fire.uf} · {fire.bioma}</p>
      ...
      <div className="overflow-y-auto flex-1 min-h-0 px-3 sm:px-4 py-3 space-y-2">
        {isLoading ? (
          ...
          <div className="text-center text-muted-foreground text-sm py-4">Nenhuma ordem de serviço.</div>
        ) : ordens.map((o: any) => (
          <article key={o.id} className="rounded-lg border border-border bg-secondary/40 hover:bg-secondary/70 transition p-3">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] mono px-1.5 py-0.5 rounded ${prioColor["P2"]}`}>P2</span>
                  <span className="mono text-[11px] text-muted-foreground">OS-{o.id}</span>
                </div>
                <p className="text-sm font-medium mt-1 truncate">{o.localizacaoTexto || 'Sem local'}</p>
                <p className="text-[11px] text-muted-foreground mono mt-0.5">Vinc. Foco-000 · ID Resp: {o.relatorId || 'N/A'}</p>
              </div>
            ...
            <div className="mt-2 flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                {["Terrestre"].map((t: any) => {
                  ...
                  return (
                    <span key={t} className="flex items-center gap-1 text-[10px] mono px-1.5 py-0.5 rounded border border-border bg-background/40">
              ...
              <span className={`text-[10px] mono uppercase tracking-wider px-1.5 py-0.5 rounded border text-muted-foreground border-border bg-secondary/60`}>
                {o.status || 'Rascunho'}
              </span>
            </div>
            <p className="text-[10px] text-muted-foreground mono mt-2">criada em {new Date(o.dataCriacao).toLocaleDateString('pt-BR')}</p>
          </article>
        ))}
      </div>
    </section>
