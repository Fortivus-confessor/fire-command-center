  ...
  return (
    <div className="glass rounded-xl p-3 sm:p-4">
      ...
      <ol className="relative">
        ...
        {timeline.length === 0 ? (
          <div className="text-center text-muted-foreground text-sm py-4">Nenhuma timeline registrada.</div>
        ) : (
          timeline.map((t, i) => (
            <li key={i} className="relative pl-6 pb-3 last:pb-0">
              <span className={`absolute left-0 top-1 h-3.5 w-3.5 rounded-full border-2 ring-2 ring-background ${toneColor[t.tone]}`} />
