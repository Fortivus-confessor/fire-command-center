...
export interface FireData {
  ...
  bioma: string;
  totalFocos: number;
  focos?: Array<{ id: string; latitude: number; longitude: number; frp: number }>;
}
...
function makeFireIcon(color: string, size: number, risco: RiskLevel) {
  ...
  return L.divIcon({
    html: `<div style="position:relative;width:${size}px;height:${size}px">
      ${pulse}
      <span style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;border-radius:50%;background:${color};box-shadow:0 0 12px 3px ${color}aa,0 0 24px 6px ${color}66;border:2px solid rgba(0,0,0,0.6)">
...
export function FireMarker({ fire, selected, onSelect }: FireMarkerProps) {
  ...
  return (
    <Marker
      ...
      icon={makeFireIcon(color, size, fire.risco)}
