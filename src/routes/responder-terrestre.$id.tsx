...
function ResponderTerrestrePage() {
  ...
  const handleSave = async (e?: React.FormEvent) => {
    ...
    try {
      const allFiles = [...selectedFiles.current.origem, ...selectedFiles.current.anexos];
      
      if (allFiles.length > 0) {
        await uploadFilesToSeaweed(allFiles, id as string, 'RELATORIO_TERRESTRE', (progress) => {
          setUploadProgress(progress);
        });
      }

      // Here you would also POST the rest of the form data to the Report Service
      
      toast.success('Relatório Terrestre Finalizado', {
        description: 'Os dados do relatório e as evidências foram enviados com sucesso.',
      });
  ...
  return (
    <div className="p-4 sm:p-6 max-w-4xl mx-auto space-y-6 pb-20">
      ...
      <div className="glass-strong rounded-xl border border-border p-4 sm:p-6">
        <RelatorioTerrestreForm onSubmit={handleSave} onFilesChange={handleFilesChange} />
      </div>
