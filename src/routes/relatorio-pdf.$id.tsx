...
function Field({ label, value }: { label: string; value?: string | number | null | boolean }) {
  return (
    <div className="py-1">
      ...
      <span>
        {value !== undefined && value !== null && value !== false ? value.toString() : "--"}
      </span>
    </div>
  );
}
...
function RelatorioPdfPage() {
  ...
  const usuariosEscalados = escala?.membrosIds
    ? escala.membrosIds.map(
        (uid: number) => usuarios.find((u) => u.id === uid)?.nome || `ID ${uid}`,
      )
    : [];

  const usuarioPreenchedor = usuarios.find(
    (u) => u.id === despacho.responsavelId || u.id === relatorio?.preenchidoPorId,
  );
    ...
    <div className="min-h-screen bg-background flex flex-col items-center">
      ...
      <div className="w-full max-w-[210mm] min-h-[297mm] bg-card text-card-foreground my-8 p-12 shadow-lg print:bg-white print:text-black print:shadow-none print:m-0 print:p-8">
        ...
        <div className="text-center border-b-2 border-foreground print:border-black pb-4 mb-4">
          ...
          <h2 className="text-base font-semibold mt-2 text-muted-foreground print:text-gray-700">
            Relatório Operacional de Despacho {tituloCategoria}
          </h2>
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="mt-2 px-2 grid grid-cols-2 gap-x-6 gap-y-1">
            ...
            <Field
              ...
              value={usuarioPreenchedor ? usuarioPreenchedor.nome : "--"}
            />
          ...
          {relatorio ? (
            <>
              ...
              {(relatorioType === "aereo" || relatorioType === "maquinario") && (
                <div className="mt-2 px-2">
                  <div className="grid grid-cols-2 gap-x-6">
                    ...
                    <Field
                      label="Tempo Líquido"
                      value={relatorio.horasLiquidas || relatorio.tempoLiquido}
                    />
                    {relatorioType === "maquinario" && (
                      <>
                  ...
                  {relatorioType === "maquinario" && relatorio.descricaoOutroEmprego && (
                    <Field
                      label="Descrição Outro Emprego"
                      value={relatorio.descricaoOutroEmprego}
                    />
                  )}
              ...
              {(relatorioType === "terrestre" || relatorioType === "aereo") && (
                <>
                  ...
                  <div className="mt-2 px-2">
                    <Field
                      label="Houve uso de água"
                      value={relatorio.houveUsoAgua ? "Sim" : "Não"}
                    />
                    {relatorio.houveUsoAgua && (
                      <>
                        ...
                        {relatorio.outraOrigemAguaDescricao && (
                          <p className="text-xs italic text-muted-foreground print:text-gray-600">
                            Descrição: {relatorio.outraOrigemAguaDescricao}
                          </p>
                        )}
                      </>
                    )}
              ...
              {relatorioType === "terrestre" && (
                <>
                  ...
                  <div className="mt-2 px-2">
                    ...
                    {relatorio.orgaosApoio?.length > 0
                      ? relatorio.orgaosApoio.map((o: string) => ORGAOS_MAP[o] ?? o).join(", ")
                      : "Nenhum"}
                    {relatorio.outrosOrgaosDescricao && (
                      <p className="text-xs italic text-muted-foreground print:text-gray-600 mt-1">
                        Outros Órgãos: {relatorio.outrosOrgaosDescricao}
                      </p>
                    )}

                    <div className="mt-4">
                      <Field
                        label="Houve apoio de propriedades"
                        value={relatorio.houveApoioPropriedades ? "Sim" : "Não"}
                      />
                      <Field
                        label="Houve recusa de propriedades"
                        value={relatorio.houveRecusaPropriedades ? "Sim" : "Não"}
                      />
                      {relatorio.propriedades
                        ?.filter((p: any) => p.tipoRegistro === "APOIO")
                        .map((p: any, i: number) => (
                          <div
                            key={i}
                            className="mt-2 pl-4 border-l-2 border-border print:border-gray-200"
                          >
                            <p className="font-medium">
                              {p.nomePropriedade || `Propriedade ${i + 1}`}
                            </p>
                            {p.responsavel && (
                              <p className="text-xs">
                                Resp.: {p.responsavel} {p.telefone && `— ${p.telefone}`}
                              </p>
                            )}
                  ...
                  <div className="mt-2 px-2">
                    <Field
                      label="Origem"
                      value={
                        relatorio.possivelOrigemIncendio
                          ? (ORIGENS_MAP[relatorio.possivelOrigemIncendio] ??
                            relatorio.possivelOrigemIncendio)
                          : "--"
                      }
                    />
                    {relatorio.outraOrigemDescricao && (
                      <p className="text-xs italic text-muted-foreground print:text-gray-600">
                        Descrição: {relatorio.outraOrigemDescricao}
                      </p>
                    )}
                  </div>
                </>
              ...
              <SectionHeader
                number={relatorioType === "terrestre" ? "8" : "6"}
                title="Efetividade e Necessidade de Reforço"
              />
              <div className="mt-2 px-2 grid grid-cols-2 gap-x-6">
                <Field
                  label="Efetividade"
                  value={
                    relatorio.efetividadeCombate
                      ? (EFETIVIDADE_MAP[relatorio.efetividadeCombate?.toUpperCase()] ??
                        relatorio.efetividadeCombate)
                      : "--"
                  }
                />
                <Field
                  ...
                  value={relatorio.necessidadeReforco ? "Sim" : "Não"}
                />
              ...
              {/* 8/9. Histórico (All) */}
              <SectionHeader
                number={relatorioType === "terrestre" ? "9" : "7"}
                title="Histórico Descritivo"
              />
              ...
              {/* 9/10. Resultado (All) */}
              <SectionHeader
                number={relatorioType === "terrestre" ? "10" : "8"}
              ...
              <div className="mt-2 px-2">
                <Field
                  label="Resultado"
                  value={
                    relatorio.resultadoOcorrencia
                      ? (RESULTADO_MAP[relatorio.resultadoOcorrencia?.toUpperCase()] ??
                        relatorio.resultadoOcorrencia)
                      : "--"
                  }
                />
                {relatorio.outroResultadoDescricao && (
                  <p className="text-xs italic text-muted-foreground print:text-gray-600">
                    Descrição: {relatorio.outroResultadoDescricao}
                  </p>
                )}
                {relatorio.kmFinal && (
                  <Field label="KM Final do Veículo" value={relatorio.kmFinal} />
                )}
              </div>
            </>
          ) : (
            ...
            <>
              ...
              <div className="mt-2 px-2 italic text-muted-foreground print:text-gray-500 border border-dashed border-border print:border-gray-300 p-4 rounded">
                Este despacho ainda não possui um relatório de campo preenchido pela equipe. Acesse
                a ação "Responder {tituloCategoria}" para preencher o relatório.
              </div>
            </>
          )}
